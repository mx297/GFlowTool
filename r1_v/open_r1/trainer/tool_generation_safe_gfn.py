...
def _resolve_tool_api_params(item: Dict[str, Any], api_params: Dict[str, Any]) -> Dict[str, Any]:
    resolved_api_params = copy.deepcopy(api_params)
    image_value = resolved_api_params.get("image")
    if not isinstance(image_value, str):
        return resolved_api_params

    # The safe validator path keeps symbolic image ids like `img_1` intact.
    # Online tools expect raw base64 image bytes instead.
    if re.fullmatch(r"(img_\d+|image-\d+)", image_value):
        current_image = item["images"][-1] if item.get("images") else None
        if current_image is not None:
            resolved_api_params["image"] = pil_to_base64(current_image, url_format=False)
    return resolved_api_params
...
def vllm_generate_with_tool_calls_gfn(
    ...
    for _ in range(max_rounds):
        ...
        for input_idx, output_text, output_ids in zip(input_idxs, output_texts, output_idss):
            ...
            api_params = _resolve_tool_api_params(item, validation.action_arguments or {})
