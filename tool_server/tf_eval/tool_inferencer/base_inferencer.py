...
class BaseToolInferencer(object):
    def __init__(
        ...
        if self.accelerator.device.type == "cuda" and not 'vllm_models' in str(type(self.tp_model)):
                ...
                try:
                    if "edited_image" in tool_response:
                        ...
                        if isinstance(edited_image, list):
                            edited_image = edited_image[0] if len(edited_image) > 0 else None
                        item.current_image = edited_image

                        if self.model_mode == "llava_plus": 
                            edited_image = base64_to_pil(edited_image) if edited_image is not None else None
                            ...
                            tool_cfg = [
                                {
                                    ...
                                    "API_params": {
                                        "ans": action_arguments.get("ans", action_arguments.get("answer", ""))
                                    },
                                }
                            ]
                        else:
