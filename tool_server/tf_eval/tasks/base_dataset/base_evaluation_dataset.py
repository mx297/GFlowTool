...
class BaseEvalDataset(Dataset):
    ...
    def __init__(
        ...
        self.full_data = self._expand_for_num_generations(self.load_data_function())
    ...
    def _expand_for_num_generations(self, full_data):
        num_generations = int(self.gen_kwargs.get("num_generations", 1) or 1)
        if num_generations <= 1:
            normalized = []
            for item in full_data:
                updated = deepcopy(item)
                updated.setdefault("prompt_idx", updated["idx"])
                updated.setdefault("sample_idx", 0)
                updated["num_generations"] = 1
        ...
        for item in full_data:
            ...
            for sample_idx in range(num_generations):
                sample_item = deepcopy(item)
                sample_item["prompt_idx"] = base_idx
                sample_item["sample_idx"] = sample_idx
                sample_item["num_generations"] = num_generations
                sample_item["idx"] = f"{base_idx}__sample_{sample_idx}"
